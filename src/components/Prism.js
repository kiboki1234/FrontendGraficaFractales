...
const fractals = {
  julia: {
    ...
    equation: <BlockMath math={"Z_{n+1} = Z_n^2 + C"} />,
    explanation: `El conjunto de Julia se define mediante una función iterativa cuadrática sobre los números complejos.`,
    theory: `Para cualquier número complejo \\( C \\), el conjunto de Julia se forma evaluando 
             si los valores de \\( Z \\) escapan a infinito bajo la transformación \\( Z_{n+1} = Z_n^2 + C \\).`,
  ...
  mandelbrot: {
    ...
    equation: <BlockMath math={"Z_{n+1} = Z_n^2 + C, \\quad Z_0 = 0"} />,
    ...
    theory: `Para cada punto en el plano complejo, la ecuación \\( Z_{n+1} = Z_n^2 + C \\) se itera hasta 
             que el valor de \\( Z \\) diverge.`,
    ...
    formulaCode: `function mandelbrot(c, maxIter) {   let z = { real: 0, imag: 0 };
  let n = 0;
  while (Math.sqrt(z.real * z.real + z.imag * z.imag) <= 2 && n < maxIter) {
      let real = z.real * z.real - z.imag * z.imag + c.real;
      let imag = 2 * z.real * z.imag + c.imag;
  ...
  koch: {
    name: "Curva de Koch",
    equation: <BlockMath math={"D_f = \\frac{\\log 4}{\\log 3} \\approx 1.26"} />,
  ...
  sierpinski: {
    ...
    equation: <BlockMath math={"A_{n+1} = \\frac{A_n}{2}"} />,
    ...
    dimension: `Su dimensión fractal es \\(\\log 3 / \\log 2 \\approx 1.58\\).`,
    properties: [
    ...
    formulaCode: `function sierpinski(points, depth) {   if (depth === 0) return [points];
  const [a, b, c] = points;
  ...
  tree: {
    name: "Árbol Fractal",
    equation: <BlockMath math={"L_{n+1} = r L_n"} />,
    ...
    formulaCode: `
    function treeFractal(start, length, angle, depth) {   if (depth === 0) return [];
        const end = {
    ...
    <div className="container text-light py-5">
      ...
      <div className="card bg-dark text-light shadow-lg p-4 mb-4">
        <div className="card-body">
          ...
          {fractals[selectedFractal].equation}
